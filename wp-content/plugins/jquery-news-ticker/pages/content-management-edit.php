...
else
{
	...
	$form = array(
		...
		'Jntp_description' => $data['Jntp_description'],
...
if (isset($_POST['Jntp_form_submit']) && $_POST['Jntp_form_submit'] == 'yes')
{
	...
	{	
		$sSql = $wpdb->prepare(
				...
				WHERE Jntp_id = %d
				LIMIT 1",
				array($form['Jntp_text'], $form['Jntp_description'], $form['Jntp_order'], $form['Jntp_status'], $form['Jntp_group'], $form['Jntp_dateend'], $did)
			);
...
<div class="form-wrap">
	...
	<form name="Jntp_form" method="post" action="#" onsubmit="return _Jntp_submit()"  >
	  ...
	  <label for="tag-a"><?php _e('Title', 'jquery-news-ticker'); ?></label>
		...
		<p><?php _e('Please Edit your news title.', 'jquery-news-ticker'); ?></p>
		
		<label for="tag-a"><?php _e('News Description', 'jquery-news-ticker'); ?></label>
    <?php 
			$content = $form['Jntp_description'];
