...
if (isset($_POST['Jntp_form_submit']) && $_POST['Jntp_form_submit'] == 'yes')
{
	...
	$form['Jntp_description'] = isset($_POST['Jntp_description']) ? $_POST['Jntp_description'] : '';
	if ($form['Jntp_description'] == '')
	{
		$Jntp_errors[] = __('Please enter your the news description.', 'jquery-news-ticker');
		$Jntp_error_found = TRUE;
	}
	...
	{
		echo $_POST['Jntp_text'];
		$Jntp_date = date('Y-m-d H:i:s');
		$sql = $wpdb->prepare(
			"INSERT INTO `".Jntp_Table."`
			(`Jntp_text`, `Jntp_description`, `Jntp_order`, `Jntp_status`, `Jntp_date`, `Jntp_group`, `Jntp_dateend`)
			VALUES(%s, %s, %s, %s, %s, %s, %s)",
			array($form['Jntp_text'], $form['Jntp_description'], $form['Jntp_order'], $form['Jntp_status'], $Jntp_date, $form['Jntp_group'], $form['Jntp_dateend'])
		);
		$wpdb->query($sql);
...
<div class="form-wrap">
	...
	<form name="Jntp_form" method="post" action="#" onsubmit="return _Jntp_submit()"  >
      <h3><?php _e('Add details', 'jquery-news-ticker'); ?></h3>
		...
		<label for="tag-a"><?php _e('Title', 'jquery-news-ticker'); ?></label>
		<textarea name="Jntp_text" id="Jntp_text" cols="90" rows="2"></textarea>
		<p><?php _e('Please enter your ticker news.', 'jquery-news-ticker'); ?></p>
		
		<label for="tag-a"><?php _e('Description', 'jquery-news-ticker'); ?></label>
 <?php 
			$content = '';
			$editor_id = 'Jntp_description';
			wp_editor( $content, $editor_id );
	?>	
		<p><?php _e('Please enter your description.', 'jquery-news-ticker'); ?></p>
